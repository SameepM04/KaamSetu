...
class _OtpVerificationScreenState extends State<OtpVerificationScreen> {
  ...
  Future<void> _verify([String? code]) async {
    ...
    try {
      // In debug mode (kDebugMode == true), WorkerAuthService accepts any
      // 6-digit code, makes no Firebase calls, and returns immediately —
      // so this simply falls through to the same navigation used for a
      // real, successful production verification. Nothing here needs to
      // branch on debug vs. production; the service already handled that.
