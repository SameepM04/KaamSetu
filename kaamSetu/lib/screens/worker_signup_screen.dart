...
class _WorkerSignUpScreenState extends State<WorkerSignUpScreen> with SingleTickerProviderStateMixin {
  ...
  late final AnimationController _photoAnim =
    ...
    try {
      await _authService.sendOtp(
        ...
        onFailed: (error) {
          ...
          setState(() {
            _submitting = false;
            _phoneError = error.message ?? 'Could not send OTP. Please try again.';
          });
        },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: const [
              Text('Profile Photo', style: TextStyle(color: AppColors.navy, fontSize: 15, fontWeight: FontWeight.w800)),
              SizedBox(width: 6),
              Text('(Optional)', style: TextStyle(color: AppColors.inkMuted, fontSize: 13)),
            ],
          ),
          ...
          Center(
            child: GestureDetector(
              ...
              child: ScaleTransition(
                ...
                child: _ProfilePhotoCircle(avatar: selectedAvatar, imageFile: galleryImage),
              ),
            ),
          ),
          const SizedBox(height: 16),
            ...
            children: [
              Expanded(
                child: _QuickPickTile(
                  label: 'Male Avatar',
                  child: ClipOval(child: Image.asset('assets/avatars/male_avatar.png', fit: BoxFit.cover)),
              ...
              Expanded(
                child: _QuickPickTile(
                  label: 'Female Avatar',
                  child: ClipOval(child: Image.asset('assets/avatars/female_avatar.png', fit: BoxFit.cover)),
              ...
              Expanded(
                child: _QuickPickTile(
                  label: 'Choose from\nGallery',
                  child: const Icon(Icons.image_rounded, color: AppColors.blue, size: 28),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('Take Photo', style: TextStyle(color: AppColors.navy, fontWeight: FontWeight.w700)),
          ...
          _AppTextField(
            ...
            errorText: nameTouched && !isNameValid ? 'Full name is required' : null,
          ),
          ...
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: const [
              Icon(Icons.verified_user_rounded, color: AppColors.blue, size: 18),
              SizedBox(width: 8),
              Expanded(
                child: Text(
                  'We will send you a 6-digit OTP to verify your number.',
                  style: TextStyle(color: AppColors.inkMuted, fontSize: 13, height: 1.35),
                ),
              ),
            ],
          ),
          const SizedBox(height: 18),
          _CreateAccountButton(enabled: isFormValid, submitting: submitting, onTap: onSubmit),
        ],
      ),
    );
  }
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: hasSelection
                ? [BoxShadow(color: AppColors.blue.withValues(alpha: .28), blurRadius: 18, spreadRadius: 1)]
                : null,
          ),
          child: ClipOval(
            child: imageFile != null
                ...
                : avatar != null
                    ...
                    : const Icon(Icons.person_outline_rounded, color: AppColors.inkMuted, size: 46),
          ),
        ),
        Positioned(
          right: -2,
          ...
          child: Container(
            ...
            decoration: const BoxDecoration(
              ...
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: Color(0x334775B4), blurRadius: 8, offset: Offset(0, 3))],
            ),
...
class _OrDivider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: const [
      ...
      Padding(padding: EdgeInsets.symmetric(horizontal: 10), child: Text('or', style: TextStyle(color: AppColors.inkMuted, fontSize: 13))),
      Expanded(child: Divider(color: AppColors.line)),
    ]);
  }
}
...
class _QuickPickTile extends StatelessWidget {
  const _QuickPickTile({required this.label, required this.child, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          border: Border.all(color: selected ? AppColors.blue : AppColors.line, width: selected ? 2 : 1.4),
          borderRadius: BorderRadius.circular(16),
          color: selected ? AppColors.paleBlue.withValues(alpha: .5) : Colors.white,
        ),
        child: Column(
          children: [
            ...
            Text(label,
                ...
                style: const TextStyle(color: AppColors.navy, fontSize: 11.5, fontWeight: FontWeight.w700, height: 1.2)),
          ],
        ),
      ),
    );
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: errorText != null ? Colors.red.shade300 : AppColors.line, width: 1.4)),
        ...
        if (errorText != null) ...[
          ...
          Padding(
            ...
            child: Text(errorText!, style: TextStyle(color: Colors.red.shade400, fontSize: 12.5)),
          ),
        ],
      ],
    );
...
class _CreateAccountButton extends StatefulWidget {
  const _CreateAccountButton({required this.enabled, required this.submitting, required this.onTap});
...
class _CreateAccountButtonState extends State<_CreateAccountButton> with SingleTickerProviderStateMixin {
