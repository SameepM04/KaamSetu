...
class _BackdropPainter extends CustomPainter {
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..shader = RadialGradient(colors: [
          AppColors.warmGold.withOpacity(glowOpacity * 0.5),
          AppColors.warmGold.withOpacity(0.0),
  ...
  void _skyline(Canvas canvas, Rect bounds) {
    final paint = Paint()..color = AppColors.blue.withOpacity(0.12);
...
class _DecorationsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _circle(canvas, Offset(w * 0.14, h * 0.20), 5, AppColors.blue.withOpacity(0.22));
    _circle(canvas, Offset(w * 0.78, h * 0.16), 7, Colors.white.withOpacity(0.8));
    _circle(canvas, Offset(w * 0.72, h * 0.09), 4, Colors.white.withOpacity(0.7));
    _circle(canvas, Offset(w * 0.20, h * 0.42), 4, AppColors.orange.withOpacity(0.28));
    _circle(canvas, Offset(w * 0.86, h * 0.36), 5, AppColors.orange.withOpacity(0.22));
    _circle(canvas, Offset(w * 0.44, h * 0.50), 4, AppColors.blue.withOpacity(0.18));
    ...
    for (var i = 0; i < 6; i++) {
      ...
      _circle(canvas, Offset(baseX + dx, baseY + dy), 2.5, AppColors.blue.withOpacity(0.14));
    }
  }
  ...
  void _dotGrid(Canvas canvas, Offset origin, {int rows = 3, int cols = 3}) {
    final paint = Paint()..color = AppColors.blue.withOpacity(0.22);
    const spacing = 12.0;
    for (var r = 0; r < rows; r++) {
      for (var c = 0; c < cols; c++) {
        canvas.drawCircle(Offset(origin.dx + c * spacing, origin.dy + r * spacing), 1.6, paint);
      }
    }
  }
  ...
  void _bird(Canvas canvas, Offset center) {
    ...
    final path = Path()
      ...
      ..quadraticBezierTo(center.dx - r / 2, center.dy - r / 2, center.dx, center.dy)
      ..quadraticBezierTo(center.dx + r / 2, center.dy - r / 2, center.dx + r, center.dy);
    canvas.drawPath(path, paint);
  }

  @override
  bool shouldRepaint(covariant _DecorationsPainter oldDelegate) => oldDelegate.t != t;
}
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  gradient: RadialGradient(
                    colors: [
                      AppColors.blue.withOpacity(0.22 * glow),
                      Colors.transparent,
                    ],
                  ),
                ),
          ...
          builder: (context, _) {
            ...
            return Stack(
              ...
              children: [
                Positioned(
                  top: h * 0.58,
                  child: _AnimatedBridge(progress: _bridge.value, width: bridgeWidth),
                ),
                Positioned(
                  left: w * 0.02,
                  bottom: 0,
                  child: _Platform(
                    ...
                    bob: workerBob,
                    child: Image.asset(_kWorker, width: w * 0.24, fit: BoxFit.contain),
                  ),
                ),
                Positioned(
                  right: w * 0.00,
                  ...
                  child: _Platform(
                    ...
                    bob: houseBob,
                    child: Image.asset(_kHouse, width: w * 0.30, fit: BoxFit.contain),
                  ),
                ),
              ],
            );
...
class _Platform extends StatelessWidget {
  const _Platform({required this.color, required this.bob, required this.child});
  final Color color;
  final double bob;
  final Widget child;
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Stack(
        alignment: Alignment.center,
        children: [
          Image.asset(_kBridgeDots, width: width, height: height, fit: BoxFit.fitWidth),
          Transform.translate(
            offset: Offset(dx, 0),
            child: Container(
              width: 9,
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: const LinearGradient(colors: [AppColors.blue, AppColors.orange]),
                boxShadow: [
                  BoxShadow(color: AppColors.blue.withOpacity(0.5), blurRadius: 8, spreadRadius: 1),
                ],
              ),
            ),
          ),
...
class SplashFeaturePanel extends StatelessWidget {
  ...
  double _stagger(double start, double end) =>
      Interval(start, end, curve: Curves.easeOut).transform(entry.value).clamp(0.0, 1.0);
            ...
            children: [
              ...
              Expanded(
                child: _FeatureCard(
                  ...
                  iconBg: AppColors.orange.withOpacity(0.14),
              ...
              Container(
                ...
                child: ClipOval(
                  child: Transform.scale(
                    scale: 1.7, // zoom past baked-in labels/decoration, center on the glyph
            ...
            RichText(
              text: const TextSpan(
                ...
                children: [
                  ...
                  TextSpan(text: 'Kaam', style: TextStyle(color: AppColors.blue, fontWeight: FontWeight.w700)),
                  TextSpan(text: 'Setu', style: TextStyle(color: AppColors.orange, fontWeight: FontWeight.w700)),
                  TextSpan(text: '...'),
                ],
              ),
            ),
