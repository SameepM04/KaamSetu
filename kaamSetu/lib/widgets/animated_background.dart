...
class _BackdropPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    glow(Offset(size.width * .78, size.height * .08), size.width * .68, const Color(0x336CB7FF));
    glow(Offset(size.width * .16, size.height * .71), size.width * .55, const Color(0x1B66B6FF));
    ...
    for (var line = 0; line < 12; line++) {
      ...
      for (double x = -20; x <= size.width + 20; x += 14) {
        final curve = math.sin((x / size.width * math.pi * 2) + time * math.pi * 2 + line * .16);
        path.lineTo(x, y + curve * (13 + line * .25));
      }
      canvas.drawPath(path, wave..color = Colors.white.withValues(alpha: .26 - line * .012));
    }

    final nodePaint = Paint()..color = const Color(0xFFB7D5FC).withValues(alpha: .3);
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      final center = Offset(size.width * particle.x + drift, size.height * particle.y - drift);
      canvas.drawCircle(center, particle.radius * 3, Paint()..color = particle.color.withValues(alpha: .08));
      canvas.drawCircle(center, particle.radius, Paint()..color = particle.color);
    }

    if (showPins) {
      final pin = Paint()..color = const Color(0xFF86B4F5).withValues(alpha: .18);
      for (final position in [Offset(size.width * .09, size.height * .26), Offset(size.width * .86, size.height * .13), Offset(size.width * .96, size.height * .33)]) {
        final path = Path()
        ...
        canvas.drawCircle(position, 3.2, Paint()..color = const Color(0xFFF8FBFF));
      }
    }
  }
