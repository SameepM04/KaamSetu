...
class RoleSelectionAmbientOverlay extends StatefulWidget {
  ...
  @override
  State<RoleSelectionAmbientOverlay> createState() => _RoleSelectionAmbientOverlayState();
}

class _RoleSelectionAmbientOverlayState extends State<RoleSelectionAmbientOverlay>
  ...
  void paint(Canvas canvas, Size size) {
    for (var i = 0; i < _particles.length; i++) {
      ...
      canvas.drawCircle(
        center,
        particle.radius,
        Paint()..color = particle.color.withValues(alpha: particle.color.a * fade),
      );
    }
  }

  @override
  bool shouldRepaint(covariant _AmbientPainter oldDelegate) => oldDelegate.time != time;
}
