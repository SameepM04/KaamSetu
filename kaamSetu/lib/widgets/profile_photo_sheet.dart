  ...
  Future<void> _pick(ImageSource source) async {
    ...
    try {
      ...
      if (file != null && mounted) {
        Navigator.of(context).pop(ProfilePhotoSelection(imageFile: File(file.path)));
      }
    } finally {
    ...
    return SafeArea(
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: Color(0x22102A54), blurRadius: 30, offset: Offset(0, 12))],
        ),
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: _AvatarOption(
                    ...
                    onTap: () => Navigator.of(context)
                        .pop(const ProfilePhotoSelection(avatar: ProfilePhotoAvatar.male)),
                  ),
                ),
                ...
                Expanded(
                  child: _AvatarOption(
                    ...
                    onTap: () => Navigator.of(context)
                        .pop(const ProfilePhotoSelection(avatar: ProfilePhotoAvatar.female)),
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
            ...
            Row(children: const [
              ...
              Padding(padding: EdgeInsets.symmetric(horizontal: 10), child: Text('or', style: TextStyle(color: AppColors.inkMuted))),
              Expanded(child: Divider(color: AppColors.line)),
            ]),
            const SizedBox(height: 14),
            SizedBox(
            ...
            SizedBox(
              ...
              child: TextButton(
                ...
                child: const Text('Cancel', style: TextStyle(color: AppColors.inkMuted, fontWeight: FontWeight.w600)),
              ),
            ),
          ],
        ),
...
class _AvatarOption extends StatelessWidget {
  const _AvatarOption({required this.label, required this.assetPath, required this.onTap});
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ClipOval(
              child: Image.asset(assetPath, width: 56, height: 56, fit: BoxFit.cover),
            ),
            const SizedBox(height: 8),
            Text(label,
                textAlign: TextAlign.center,
                style: const TextStyle(color: AppColors.navy, fontSize: 12, fontWeight: FontWeight.w700)),
          ],
        ),
      ),
    );
...
class _ActionOption extends StatelessWidget {
  const _ActionOption({required this.icon, required this.label, required this.onTap, this.busy = false});
