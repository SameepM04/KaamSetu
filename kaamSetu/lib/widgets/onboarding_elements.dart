    ...
    return Transform.translate(
      ...
      child: DecoratedBox(
        decoration: const BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Color(0x334B82FB), blurRadius: 18, offset: Offset(0, 10))],
        ),
        child: ClipOval(
          child: Image.asset(
  ...
  Widget build(BuildContext context) {
    ...
    return Transform.translate(
      ...
      child: DecoratedBox(
        decoration: const BoxDecoration(
          shape: BoxShape.circle,
          boxShadow: [BoxShadow(color: Color(0x33F79B4E), blurRadius: 18, offset: Offset(0, 10))],
        ),
        child: ClipOval(
          child: Image.asset(
...
class _BridgePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 6; i++) {
      ...
      final x = size.width * .5 + direction * (end - start) * (.5 - phase * .45);
      final radius = (i == 0 ? 6 : 3.2) + progress * 1.8;
      canvas.drawCircle(Offset(x, y), radius, dotPaint..color = AppColors.electricBlue.withValues(alpha: .45 + progress * .55));
    }
  }

  @override
  bool shouldRepaint(covariant _BridgePainter oldDelegate) => oldDelegate.time != time || oldDelegate.progress != progress;
}
...
class TrustConnection extends StatelessWidget {
  const TrustConnection({super.key, required this.progress, required this.ambient});
  final double progress;
  final double ambient;

  @override
  Widget build(BuildContext context) => RepaintBoundary(
        child: CustomPaint(painter: _TrustPainter(progress: progress, ambient: ambient), child: const SizedBox.expand()),
      );
}
...
class _TrustPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, glow, Paint()..shader = RadialGradient(colors: [const Color(0xFFFFD16C).withValues(alpha: .45), const Color(0x00FFD16C)]).createShader(Rect.fromCircle(center: center, radius: glow)));
    for (var i = 0; i < 3; i++) {
      final radius = 38 + i * 22 + progress * 15;
      canvas.drawCircle(center, radius, Paint()..style = PaintingStyle.stroke..strokeWidth = 1.2..color = const Color(0xFFFFD98B).withValues(alpha: (1 - progress) * .38));
    }
    final bridge = Paint()..color = AppColors.electricBlue..strokeWidth = 2.5..strokeCap = StrokeCap.round;
    canvas.drawLine(Offset(size.width * .23, size.height * .78), Offset(size.width * (.23 + .54 * progress), size.height * .78), bridge);
    ...
    final pin = Path()
      ...
      ..close();
    canvas.drawPath(pin, Paint()..color = Colors.white.withValues(alpha: progress));
    if (progress > .35) {
      final orbit = 38 + (1 - progress) * 36;
      for (var i = 0; i < 4; i++) {
        final angle = ambient * math.pi * 2 + i * math.pi / 2;
        final point = pinCenter + Offset(math.cos(angle) * orbit, math.sin(angle) * orbit);
        canvas.drawCircle(point, 6, Paint()..color = [AppColors.blue, AppColors.orange, AppColors.green, const Color(0xFFFFC75E)][i].withValues(alpha: (1 - progress) * 1.4));
      }
    }
    final left = RRect.fromRectAndRadius(Rect.fromLTWH(-8, size.height * .53, size.width * .51, size.height * .18), const Radius.circular(30));
    final right = RRect.fromRectAndRadius(Rect.fromLTWH(size.width * .49, size.height * .53, size.width * .51, size.height * .18), const Radius.circular(30));
    canvas.drawRRect(left, Paint()..color = const Color(0xFF4C85EE));
    canvas.drawRRect(right, Paint()..color = const Color(0xFFFF9A49));
    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center: center, width: 76, height: 43), const Radius.circular(22)), Paint()..color = const Color(0xFFF1B28A));
  }
  @override
  bool shouldRepaint(covariant _TrustPainter oldDelegate) => oldDelegate.progress != progress || oldDelegate.ambient != ambient;
}
          ...
          child: Padding(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: index == active ? AppColors.electricBlue : const Color(0xFFCBD7E9),
...
class TrustFeatureCard extends StatelessWidget {
  const TrustFeatureCard({super.key, required this.icon, required this.label, required this.color, required this.visible});
  final IconData icon;
  final String label;
  final Color color;
  final bool visible;
          ...
          child: GlassCard(
            ...
            child: Column(
              children: [
                ...
                Text(label, textAlign: TextAlign.center, style: const TextStyle(color: AppColors.navy, fontSize: 13, height: 1.15, fontWeight: FontWeight.w700)),
                const SizedBox(height: 9),
                Container(width: 24, height: 4, decoration: BoxDecoration(color: color, borderRadius: BorderRadius.circular(3))),
              ],
            ),
          ),
        ),
...
class _PressableCtaState extends State<PressableCta> with SingleTickerProviderStateMixin {
  late final AnimationController _shine;
  bool _pressed = false;
  bool _hovering = false;
  @override
  void initState() { super.initState(); _shine = AnimationController(vsync: this, duration: const Duration(seconds: 5))..repeat(); }
  @override
        ...
        child: GestureDetector(
          ...
          child: AnimatedScale(
            ...
            child: Container(
              height: 75,
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [Color(0xFF245DE5), Color(0xFF2375FF)]),
                borderRadius: BorderRadius.circular(39),
                boxShadow: [BoxShadow(color: const Color(0xFF3267E8).withValues(alpha: _pressed ? .62 : .42), blurRadius: _pressed ? 29 : 20, offset: const Offset(0, 10))],
                border: Border.all(color: Colors.white.withValues(alpha: .65), width: 1.4),
              ),
              child: ClipRRect(
                borderRadius: BorderRadius.circular(39),
                child: Stack(
                  alignment: Alignment.center,
                  children: [
                    Positioned(left: -120 + _shine.value * 490, child: Transform.rotate(angle: -.28, child: Container(width: 42, height: 112, color: Colors.white.withValues(alpha: .2)))),
                    Padding(
